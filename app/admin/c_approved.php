...
$dbs = "SELECT * FROM afcard WHERE status = '1'";
$extractx = mysql_query ($dbs);
$numrows = mysql_num_rows ($extractx);


$db = "SELECT * FROM afcard WHERE status = '1' ORDER BY id DESC LIMIT $i, 50";
...
?>
		...
		<div class="tab_container">
			...
			?>
				<tr>
                    ...
                    <Td><a target="_blank" href="http://www.facebook.com/<?php echo $row['uid'];?>"><img src="http://graph.facebook.com/<?php echo $row['uid'];?>/picture" border="0"></a></Td>
    				...
    				<td><input onClick="window.location='c_approved_d.php?user=<?php echo $row['id'];?>'" type="image" src="images/icn_edit.png" title="Details">
                    
                    <form method="post" action="c_approved.php">
                    <input type="hidden" value="<?php echo $row['id'];?>" name="user">
                    <input name="remove_card" type="submit" value="" style="background-image:url(images/icn_trash.png); background-repeat:no-repeat; background-color:transparent; border:none; width:16px; height:14px;" src="images/icn_trash.png" title="Trash" onClick="if (confirm('Do you want to delete this user?') == false) return false; ">
                    </form></td>
				</tr> 
			<?php
			}
	...
	if ($y == 1)
	{
		...
		while ( ($count < $lim) && ($y < $x) )
		{
			...
			$y += 1;
			$count += 1;?>
		<form name="poll_page" action="c_c_approved.php" method="post">
	...
	else
	{
		if( ( $y - 1) >= 1)
		{
			...
			{
			?>
            <form name="poll_page" action="c_approved.php" method="post">
			<td width="30px">
